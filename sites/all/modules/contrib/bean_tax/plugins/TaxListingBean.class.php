...
class TaxListingBean extends BeanPlugin {
  ...
  public function view($bean, $content, $view_mode = 'default', $langcode = NULL) {
    ...
    if (!empty($bean->filters['vocabulary']) && (isset($active_entity['terms']) && count($active_entity['terms']))) {
      $terms = taxonomy_term_load_multiple(array_keys($active_entity['terms']));
      $i = 0;
      $content['terms'] = array();
      // Parse terms from correct vocabularies, limit list to X results.
      foreach ($terms as $term) {
        if (in_array($term->vid, $bean->filters['vocabulary']) && $i < $bean->settings['records_shown']) {
